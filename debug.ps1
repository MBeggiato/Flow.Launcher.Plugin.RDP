...
if (Test-Path $flowLauncherExe) {
    ...
    if (Test-Path "$AppDataFolder\FlowLauncher\Plugins\RDP") {
        Remove-Item -Recurse -Force "$AppDataFolder\FlowLauncher\Plugins\RDP"
    }

    Copy-Item "bin\Debug\win-x64\publish" "$AppDataFolder\FlowLauncher\Plugins\" -Recurse -Force
    Rename-Item -Path "$AppDataFolder\FlowLauncher\Plugins\publish" -NewName "RDP"
